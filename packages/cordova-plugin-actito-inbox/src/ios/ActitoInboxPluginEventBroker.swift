...
class ActitoInboxPluginEventBroker {
    ...
    static private func processQueue() {
        guard let consumer = consumer else {
            loggerInbox.debug("Cannot process event queue without a consumer.")
            return
        }

        guard !eventQueue.isEmpty else { return }

        loggerInbox.debug("Processing event queue with \(eventQueue.count) items.")

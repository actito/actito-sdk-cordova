...
class ActitoPushPlugin : CDVPlugin {
    ...
    @objc func hasRemoteNotificationsEnabled(_ command: CDVInvokedUrlCommand) {
        let result = CDVPluginResult(status: .ok, messageAs: Actito.shared.push().hasRemoteNotificationsEnabled)
        self.commandDelegate!.send(result, callbackId: command.callbackId)
    }
    ...
    @objc func allowedUI(_ command: CDVInvokedUrlCommand) {
        let result = CDVPluginResult(status: .ok, messageAs: Actito.shared.push().allowedUI)
        self.commandDelegate!.send(result, callbackId: command.callbackId)
    }

...
class ActitoPushPlugin : CordovaPlugin() {
    ...
    private val allowedUIObserver = Observer<Boolean> { allowedUI ->
        ...
        ActitoPushPluginEventBroker.dispatchEvent("notification_settings_changed", allowedUI)
    }

    private val subscriptionObserver = Observer<ActitoPushSubscription?> { subscription ->
        ActitoPushPluginEventBroker.dispatchEvent("subscription_changed", subscription?.toJson())
    }
    ...
    private fun registerListener(@Suppress("UNUSED_PARAMETER") args: CordovaArgs, callback: CallbackContext) {
        ActitoPushPluginEventBroker.setup(preferences, object : ActitoPushPluginEventBroker.Consumer {
            override fun onEvent(event: ActitoPushPluginEventBroker.Event) {
